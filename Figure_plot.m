...
global x_Stabilization x_Sontag x_QP x_Ming_kappa1 x_Ming_kappa2 x_Ming_kappa3 x_Ming_kappa4 x_Ming_kappa5
global u_Sontg_save u_QP_save u_kappa1_save u_kappa2_save u_kappa3_save u_kappa4_save u_kappa5_save
...
for i=1:1:size(Initial_position,2)
    ...
    h_7(i)=plot(x_Ming_kappa5(:,1,i),x_Ming_kappa5(:,2,i),'MarkerEdge',orng,'linewidth',1.5);
    h_8(i)=plot(x_Sontag(:,1,i),x_Sontag(:,2,i),'r-','linewidth',1.5);
end
...
lgd = legend([h8,h_1,h_2,h_3,h_4,h_5,h_6,h_7,h_8],'Obstacle','Open Loop','QP Controller','Tun-$\kappa_{1}$','Tun-$\kappa_{2}$','Tun-$\kappa_{3}$','Tun-$\kappa_{4}$','Tun-$\kappa_{5}$','Sontag','interpreter','latex');
lgd.NumColumns = 5;  % Set the number of columns in the legend
...
for i=1:size(Initial_position,2)
    ...
    h_6(i)=plot(vecnorm(u_kappa5_save(1:num_plot_end,:).'),'MarkerEdge',orng,'linewidth',1.5);
    h_7(i)=plot(vecnorm(u_Sontg_save(1:num_plot_end,:).'),'r-','linewidth',1.5);
    h_8(i)=plot(ones(size(u_kappa2_save,1),1)*3.5,'--','linewidth',1.5);
end
% Add an arrow using quiver with a larger arrowhead
arrowLength = 0.2;  % Length of the arrow
arrowDirection = [1, 1];  % Direction of the arrow
